...
sql = "select distinct(no),time from material where time >'2019-08-30 13:16:21' and time < '2019-08-3 16:16:21' and no not like('%空飞杆循环') and no not like ('%空杆回调') and no not like '%退镀';"
lst = table.query_tuple_table(conn,sql)


print(lst)
i= 0
writer = pd.ExcelWriter('moni_配方超时11.xlsx')
...
df_total.to_excel('moni_配方超时.xls')
